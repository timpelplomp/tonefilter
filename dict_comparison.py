...
def create_both(raw_neg_dict, raw_pos_dict, path="final_dict"):
    ...
    freq_pos_dict = {key: (value/raw_pos_dict_size)*100 for (key, value) in raw_pos_dict}
    freq_neg_dict = {key: (value / raw_neg_dict_size) * 100 for (key, value) in raw_neg_dict}
...
def cut_off_dict(input_dict, compare_dict, tolerance=10):
    new_dict = {key: value for (key, value) in input_dict
                if value not in compare_dict or value > compare_dict[key] + tolerance}

    return new_dict


def wrap():
    pos_dict = json.load("resources/dicts/neg/draculafirst.json")
    neg_dict = json.load("dasdasdsa")

    create_both(neg_dict, pos_dict)
